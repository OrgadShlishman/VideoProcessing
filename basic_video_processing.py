...
def convert_video_to_grayscale(input_video_path: str,
    ...
    params = get_video_parameters(input_video)

    # creating video writer object
    output_video = cv2.VideoWriter(output_video_path, params["fourcc"], params["fps"], (params["width"], params["height"]), isColor=False)
...
def convert_video_to_black_and_white(input_video_path: str,
    ...
    # creating video writer object
    output_video = cv2.VideoWriter(output_video_path, params["fourcc"], params["fps"],
    ...
    while (input_video.isOpened()):
        ...
        if ret:
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
            _, b_and_w_frame = cv2.threshold(gray_frame, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            output_video.write(b_and_w_frame)

        else:
            # Frame was not read properly
            break
