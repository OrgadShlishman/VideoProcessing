...
def create_grad_x_and_grad_y(
    ...
    Ixs = np.concatenate((np.zeros((height,1)),np.copy(input_image[:,0:width-1])),axis = 1)
    Iys = np.concatenate((np.zeros((1,width)),np.copy(input_image[0:height-1,:])),axis = 0)

    Ix_z = np.zeros_like(input_image)
    Iy_z = np.zeros_like(input_image)
...
def our_harris_corner_detector(input_image: np.ndarray, K: float,
    ...
    nrows, ncols = response_image.shape
    #convert the response image to  tiles of 25x25
    response_image_tiles = black_and_white_image_to_tiles(response_image, 25, 25)
    #For each tile, create a new tile which is all zeros except from the maximal value
    for i in range(response_image_tiles.shape[0]):
      max_ind = np.argmax(response_image_tiles[i].flatten())
      mask = np.zeros(response_image_tiles[i].shape)
      max_un = np.unravel_index(max_ind,mask.shape)
      mask[max_un[0],max_un[1]] = response_image_tiles[i].flatten()[max_ind]
      response_image_tiles[i] = mask
    #Convert the result tiles-tensor back to an image. Use
    output_image = image_tiles_to_black_and_white_image(response_image_tiles,nrows, ncols)
    #Create a zeros matrix of the shape of the original image, and place ones where the image from (3) is larger than the threshold
    output_image = np.where(output_image>threshold,1,output_image)
